...
for n_bits in b:
    ...
    ba_size = asizeof.asizeof(bit_string_array)
    size_list.append(ba_size)
    fp_counter = 0
    for _ in range(fp_attempt):
        wrong_h = random.randint(0, 2**n_bits-1)
        if bit_string_array[wrong_h] == 1:
            fp_counter += 1
    false_positive_prob.append(fp_counter/fp_attempt)
